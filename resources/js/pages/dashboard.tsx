...
const statusColors: Record<string, { color: string; icon: ReactNode }> = {
    upcoming: { color: 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white', icon: <FilePenLine className="w-4 h-4" /> },
    ongoing: { color: 'bg-gradient-to-r from-green-500 to-emerald-600 text-white', icon: <CheckCircle className="w-4 h-4" /> },
    completed: { color: 'bg-gradient-to-r from-orange-500 to-red-600 text-white', icon: <FilePenLine className="w-4 h-4" /> },
    cancelled: { color: 'bg-gradient-to-r from-red-500 to-pink-600 text-white', icon: <CheckCircle className="w-4 h-4" /> },
};

const statusBlogConfig: Record<string, { color: string; icon: ReactNode }> = {
  draft: { color: 'bg-amber-100 text-amber-800', icon: <FilePenLine className="w-4 h-4" /> },
  published: { color: 'bg-emerald-100 text-emerald-800', icon: <CheckCircle className="w-4 h-4" /> },
};
...
export default function Dashboard({
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs} user={user}>
      ...
      <motion.div className="flex h-full flex-1 flex-col gap-6 p-6 bg-gray-50" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}>
        {/* Header */}
              ...
              {recentProjects.length > 0 && (
                <Section title="Recent Projects" link="/projects" dark>
                  {recentProjects.map(p => (
                    <Link key={p.id} href={route('projects.show', p.id)} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 group border">
                      {p.image_url ? <img src={p.image_url} alt={p.title} className="w-12 h-12 rounded-lg object-cover" /> :
                        <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center"><BriefcaseBusiness className="w-6 h-6 text-gray-400" /></div>}
            ...
            <div className="bg-white rounded-xl shadow-2xl shadow-gray-800 border border-gray-200 p-5">
              {recentEvents.length > 0 && (
                <Section title="Upcoming Events" link="/events" className="">
                  {recentEvents.map(e => (
                    <Link key={e.id} href={route('events.show', e.id)} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 group border">
                      {e.image_url ? <img src={e.image_url} alt={e.title} className="w-12 h-12 rounded-lg object-cover" /> :
                        <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center"><CalendarCheck className="w-6 h-6 text-gray-400" /></div>}
                      <div className="flex-1 min-w-0">
                      ...
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${statusColors[e.status]?.color || 'bg-gray-100 text-gray-800'}`}>
                        {statusBlogConfig[e.status]?.icon}
                        {e.status}
                      </span>
              ...
              {recentApplicants.length > 0 && (
                <Section title="Job Applicants" link="/applicants">
                  {recentApplicants.map(a => (
                    <Link key={a.id} href={route('applicants.show', a.id)} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 group border">
            ...
            <div className="bg-white rounded-xl shadow-2xl shadow-gray-800 border border-gray-200 p-5">
              {recentLeads.length > 0 && (
                <Section title="Recent Leads" link="/leads">
                  {recentLeads.map(l => (
                    <Link key={l.id} href={route('leads.index')} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 group border">
                      <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
            ...
            <div className="bg-white rounded-xl shadow-2xl shadow-gray-800 border border-gray-200 p-5">
              {recentBlog.length > 0 && (
                <Section title="Upcoming Events" link="/blogs/blogcardpage" className="">
                  {recentBlog.map(b => (
                    <Link key={b.id} href={route('blogs.show', b.id)} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 group border">
                      {b.featured_image ? <img src={b.featured_image} alt={b.title} className="w-12 h-12 rounded-lg object-cover" /> :
                        <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center"><CalendarCheck className="w-6 h-6 text-gray-400" /></div>}
                      <div className="flex-1 min-w-0">
              ...
              {recentHolidays.length > 0 && (
                <Section title="Upcoming Holidays" link="/holidays">
                  {recentHolidays.map(h => (
                    <Link key={h.id} href={route('holidays.show', h.id)} className="block p-3 rounded-lg hover:bg-gray-50 group border">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          <div className="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center mr-3"><CalendarDays className="w-5 h-5 text-red-600" /></div>
                          <div>
              ...
              {recentDepartments.length > 0 && (
                <Section title="Departments" link="/department">
                  {recentDepartments.map(d => (
                    <Link key={d.id} href="/department" className="block p-3 rounded-lg hover:bg-gray-50 group border">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          <div className="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center mr-3"><Building className="w-5 h-5 text-indigo-600" /></div>
                          <div>
            ...
            <Section title="Quick Actions" link="#">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <Link key={a.href} href={a.href} className={`flex flex-col items-center justify-center p-4 rounded-lg bg-${a.color}-50 hover:bg-${a.color}-100 text-${a.color}-700 border border-${a.color}-100`}>
